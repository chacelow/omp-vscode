...
test("keeps the session event stream open through the idle grace window", () => {
  ...
  assert.match(
    sendSource,
    /if \(promptRequestStarted && sentSessionId\) \{[\s\S]*?waitForPromptSettlement/
  );
  assert.match(
    sendSource,
    /if \(promptRequestStarted && sentSessionId\) \{[\s\S]*?return;[\s\S]*?\}[\s\S]*?closeEvents\(\)/
  );
});
...
test("reuses an open event stream and hides an empty agent phase", () => {
  ...
  assert.match(
    ensureSource,
    /attempt\?\.source === current && attempt\.pending/
  );
  assert.match(
    chatWindowSource,
