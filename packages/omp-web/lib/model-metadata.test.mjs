...
test("extracts nested provider metadata", () => {
  assert.deepEqual(
    extractModelMetadata({
      limits: { max_context_length: 200000, max_completion_tokens: 16384 },
    }),
    ...
    extractMatchingModelMetadata(
      {
        data: [
          { id: "other" },
          { id: "DeepSeek-V4", contextWindow: 1000000, maxTokens: 20000 },
        ],
      },
      "deepseek-v4"
    ),
...
test("returns no metadata for catalogs without limits", () => {
  assert.deepEqual(
    extractMatchingModelMetadata({ data: [{ id: "model" }] }, "model"),
    {}
  );
});
