        ...
        const transformStream = new TransformStream({
          transform(chunk, controller) {
              ...
              if (line.startsWith("data: ")) {
                ...
                try {
                  ...
                  if (parsed && typeof parsed === "object" && parsed.response) {
                    ...
                    if (
                      respObj.candidates &&
                      Array.isArray(respObj.candidates)
                    ) {
            ...
            if (outLines.length > 0)
              controller.enqueue(
                new TextEncoder().encode(outLines.join("\n") + "\n\n")
              );
          },
          flush(controller) {
            if (buffer.trim())
...
export async function applyOmpRuntimeCredentials(
  ...
  const customProvidersData = (readOmpModelsConfig(agentDir).providers ??
    {}) as Record<string, { apiKey?: string; key?: string }>;
  for (const [provider, config] of Object.entries(customProvidersData)) {
    const key = config.apiKey || config.key;
    if (key && typeof key === "string" && key.trim()) {
      await modelRuntime.setRuntimeApiKey(provider, key.trim(), {
        allowNetwork: false,
      });
    }
  }
...
export function parseOmpDefaultModel(
  config: OmpConfig
    ...
    if (Array.isArray(tiers)) {
      for (const tier of tiers) {
        if (tier && typeof tier === "object") {
          ...
          if (typeof tierObj.inputTokensAbove !== "number")
            tierObj.inputTokensAbove = 0;
          for (const key of MODEL_COST_RATE_KEYS) {
...
function inferContextWindow(modelId: string): number {
  ...
  if (
    ...
    return 262144;
  if (id.includes("gpt-5") || id.includes("gpt-4.1") || id.includes("gpt-4o"))
    return 128000;
  return 128000;
}

export function syncOmpRuntimeModelsJson(
  agentDir: string = getOmpAgentDir()
): string {
  ...
  if (existsSync(dbPath)) {
    try {
      ...
      for (const row of rows) {
        try {
          const rawModels = JSON.parse(row.models) as Array<
          ...
          const sanitizedModels = rawModels.map((m) => {
            ...
            if (
              ...
              cleaned.contextWindow = inferContextWindow(
                String(cleaned.id || "")
              );
            }
            return cleaned;
          ...
          if (row.provider_id === "google-antigravity") {
            ...
            if (!providerObj.baseUrl)
              providerObj.baseUrl = "https://daily-cloudcode-pa.googleapis.com";
  ...
  const cachedProviderIds = new Set(
    Object.keys(providers).map((providerId) => providerId.toLowerCase())
  );
  for (const [providerId, rawConfig] of Object.entries(configuredProviders)) {
    ...
    if (providerId === "google-antigravity") {
      ...
      if (!providerConfig.baseUrl)
        providerConfig.baseUrl = "https://daily-cloudcode-pa.googleapis.com";
    ...
    const cachedProviderId = Object.keys(providers).find(
      (candidate) => candidate.toLowerCase() === providerId.toLowerCase()
    );
    const dbModelList =
      ((cachedProviderId
        ? providers[cachedProviderId]?.models
    ...
    const mergedProvider = {
      ...(cachedProviderId ? providers[cachedProviderId] : {}),
      ...providerConfig,
      models: mergedModels,
    };
    providers[providerId] = mergedProvider;
    customProviders[providerId] = mergedProvider;
    if (cachedProviderId && cachedProviderId !== providerId)
      delete providers[cachedProviderId];
  ...
  for (const providerConfig of Object.values(providers)) {
    ...
    if (Array.isArray(providerModels)) {
      for (const model of providerModels) {
        if (model && typeof model === "object")
          normalizeModelCostForSchema(model as Record<string, unknown>);
      }
    }
  }
