...
import {
  ...
  syncOmpRuntimeModelsJson,
  applyOmpRuntimeCredentials,
} from "../../../lib/omp-models";
...
async function loadModels(cwd: string): Promise<ModelsData> {
    ...
    if (Array.isArray(pConfig.models)) {
      for (const m of pConfig.models) {
        if (
          m &&
          m.id &&
          !availableArr.some((x) => x.provider === provider && x.id === m.id)
        ) {
          availableArr.push({
            provider,
            id: m.id,
  ...
  const authedProviderIds = new Set(
    ompCredentials.map((c: { provider: string }) => c.provider)
  );
  ...
  const roleModelEntries = buildVisibleRoleModels(
    roles,
    available,
    authedProviderIds
  );
  ...
  for (const entry of roleModelEntries) {
    const existing = combinedList.find(
      (x) => x.provider === entry.provider && x.id === entry.id
    );
    if (!existing) {
  ...
  for (const m of modelList) {
    ...
    thinkingLevels[key] = getSupportedThinkingLevels(
      m as unknown as Parameters<typeof getSupportedThinkingLevels>[0]
    );
  }
