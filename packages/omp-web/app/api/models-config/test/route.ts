...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!modelId)
      return NextResponse.json(
        { ok: false, error: "Model ID is required", stage: "configuration" },
        { status: 400 }
      );
    ...
    if (providerError) {
      return NextResponse.json(
        { ok: false, error: providerError, stage: "configuration" },
        { status: 400 }
      );
    }
    ...
    const configuredProvider =
      readOmpModelsConfig(getOmpAgentDir()).providers?.[providerName];
    const configuredKey = configuredProvider?.apiKey;
    if (typeof configuredKey === "string" && configuredKey.trim()) {
      await modelRuntime.setRuntimeApiKey(providerName, configuredKey.trim(), {
        allowNetwork: false,
      });
    }
    ...
    if (!resolved?.auth.apiKey) {
      const ompCredential = getUsableOmpRuntimeCredentials().find(
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Stored OMP credential could not be applied to this provider",
            stage: "credentials",
          },
          { status: 400 }
        );
