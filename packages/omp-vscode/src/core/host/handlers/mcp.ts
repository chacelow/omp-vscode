...
async function runOmp(args: string[]): Promise<McpResult> {
  ...
  try {
    ...
    return {
      ...
      error: error.trim() || `omp exited with code ${code ?? "unknown"}`,
    };
...
export function mcpAddHandler(
  params: McpAddParams
): Promise<McpResult> | McpResult {
  const args = ["mcp", "add", params.name, "--scope", params.scope];
  if (params.transport === "stdio") {
    if (!params.command)
      return { ok: false, error: "A command is required for stdio servers" };
    args.push("--", params.command, ...(params.args ?? []));
  } else {
    if (!params.url)
      return { ok: false, error: "A URL is required for HTTP and SSE servers" };
    args.push("--url", params.url, "--transport", params.transport);
  }
  return runOmp(args);
}

export function mcpTestHandler(params: {
  name: string;
  scope?: "user" | "project";
}): Promise<McpResult> {
  return runOmp([
...
export function mcpRemoveHandler(params: {
  name: string;
  scope?: "user" | "project";
}): Promise<McpResult> {
  return runOmp([
