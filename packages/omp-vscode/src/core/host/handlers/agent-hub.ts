...
export const sessionTailHandler: Handler<"sessionTail"> = ({
  sessionId,
  sinceRevision,
}) => {
  ...
  const start =
    sinceRevision === null
      ? 0
      : Math.max(0, Math.min(sinceRevision, lines.length));
...
export const agentSaveHandler: Handler<"agentSave"> = ({
  name,
  definition,
}) => {
  if (!/^[a-zA-Z0-9_-]+$/.test(name))
    throw new Error(
      "Agent names may only contain letters, numbers, underscores, and hyphens"
    );
  const directory = `${getOmpAgentDir()}/agents`;
