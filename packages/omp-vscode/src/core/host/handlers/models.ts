...
export const modelsGetHandler: Handler<"modelsGet"> = () => {
  ...
  const roleProviders = new Set(
    Object.values(modelRoles).map((r) => r.provider)
  );
  const configProviders = new Set(configItems.map((m) => m.provider));
  const dbItems = readOmpModelsFromDb().filter(
    (m) => roleProviders.has(m.provider) && !configProviders.has(m.provider)
  );

  const items = [...configItems, ...dbItems];
  const modelList = items.map((m) => ({
    id: m.id,
    name: m.name,
    provider: m.provider,
