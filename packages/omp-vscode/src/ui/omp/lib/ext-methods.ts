...
const OmpChatsByCwdSchema = z.object({ sessions: z.array(OmpSessionSummarySchema) });
...
export type OmpExtensionsToggleResult = z.infer<typeof OmpExtensionsToggleSchema>;
...
export function acpExt<T>(method: string, params: Record<string, unknown>): Promise<T> {
  return acpRequest({ type: "acp/extMethod", method, params }) as unknown as Promise<T>;
}

export async function ompSessionsListAll(limit?: number): Promise<OmpSessionsListAllResult> {
  return parseWithSchema(OmpSessionsListAllSchema, await acpExt<unknown>("_omp/sessions/listAll", { limit }));
}

export async function ompProjectsList(): Promise<OmpProjectsListResult> {
  return parseWithSchema(OmpProjectsListSchema, await acpExt<unknown>("_omp/projects/list", {}));
}

export async function ompChatsByCwd(cwd: string, limit?: number): Promise<OmpChatsByCwdResult> {
  return parseWithSchema(OmpChatsByCwdSchema, await acpExt<unknown>("_omp/chats/byCwd", { cwd, limit }));
}

export async function ompUsage(): Promise<OmpUsageResult> {
  return parseWithSchema(OmpUsageSchema, await acpExt<unknown>("_omp/usage", {}));
}

export async function ompExtensions(cwd?: string): Promise<OmpExtensionsResult> {
  return parseWithSchema(OmpExtensionsSchema, await acpExt<unknown>("_omp/extensions", { cwd }));
}

export async function ompExtensionsToggle(providerId: string, enabled?: boolean): Promise<OmpExtensionsToggleResult> {
...
export async function speechModelsList(): Promise<SpeechModelsListResult> {
  return parseWithSchema(SpeechModelsListSchema, await acpExt<unknown>("speech.models.list", {}));
}
