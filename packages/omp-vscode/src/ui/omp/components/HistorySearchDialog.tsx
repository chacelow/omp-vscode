...
export function HistorySearchDialog({ items, onClose, onSelect }: Props) {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[200] flex items-start justify-center bg-black/40 p-8"
    >
          ...
          onKeyDown={(event) => {
            ...
            if (event.key === "ArrowDown") {
              ...
              setActiveIndex((value) =>
                Math.min(matches.length - 1, value + 1)
              );
            }
        ...
        <div className="mt-3 overflow-y-auto">
          {matches.map((item, index) => (
            <button
              ...
              className={`block w-full rounded p-2 text-left ${index === activeIndex ? "bg-[var(--bg-selected)]" : "hover:bg-[var(--bg-hover)]"}`}
            >
              {item}
            </button>
          ))}
