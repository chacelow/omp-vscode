...
export const HistoryMenu = memo(function HistoryMenu({
  ...
  return (
    <div className="shadow-[0_-6px_20px_var(--vscode-widget-shadow, rgba(0,0,0,0.12))] absolute inset-x-0 bottom-[calc(100%+8px)] z-[120] max-h-[min(44vh,360px)] overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--bg)]">
      ...
      <div
        ...
        style={{ maxHeight: "calc(min(44vh, 360px) - 31px)" }}
      >
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <span className="shrink-0 pt-px font-mono text-[11px] text-[var(--text-dim)]">
                {index + 1}
              </span>
              <span
                className="break-anywhere min-w-0"
                style={{
                  display: "-webkit-box",
