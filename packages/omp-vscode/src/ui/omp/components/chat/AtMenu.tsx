...
export const AtMenu = memo(function AtMenu({
  loading, matches, activeIndex, serverResultInUse, needsServerSearch,
  indexTruncated, query, onApply, onHover, itemRefs, t,
}: AtMenuProps) {
  ...
  const truncatedHint = indexTruncated && !serverResultInUse
    ? (query ? t("chat.searchingAll") : t("chat.indexTruncated"))
    : "";
  ...
  return (
    <div className="absolute inset-x-0 bottom-[calc(100%+8px)] z-[120] max-h-[min(48vh,400px)] overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--bg)] shadow-[0_-6px_20px_var(--vscode-widget-shadow, rgba(0,0,0,0.12))]">
      ...
      <div className="overflow-y-auto p-1" style={{ maxHeight: "calc(min(48vh, 400px) - 34px)" }}>
          ...
          matches.map((entry, index) => {
            ...
            return (
              <button
                ...
                type="button"
                onMouseDown={(e) => { e.preventDefault(); onApply(entry); }}
              ...
              >
                ...
                <span className="min-w-0 truncate">
                  {dirPrefix && <span className="text-[var(--text-dim)]">{dirPrefix}</span>}
                  {name}
                  {entry.isDir && <span className="text-[var(--text-dim)]">/</span>}
                </span>
              </button>
            );
          })
