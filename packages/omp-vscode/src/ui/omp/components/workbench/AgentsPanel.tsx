  ...
  return isUserAgent ? (
    <span
      style={{
        padding: "1px 5px",
        borderRadius: 8,
        background: "var(--vscode-badge-background, var(--bg-hover))",
        color: "var(--vscode-badge-foreground, var(--text-muted))",
        fontSize: 10,
      }}
    >
      user
...
function AgentEditor({
  ...
  const { t } = useI18n();
  const field = (key: keyof AgentDraft, value: string): void =>
  ...
  return (
    ...
    >
      ...
      >
        {t("agents.field.tools")}
        <input
          value={draft.tools}
          onChange={(event) => field("tools", event.target.value)}
          style={inputStyle}
        />
      </label>
      <label
      ...
      >
        {t("agents.field.isolation")}
        <select
          value={draft.isolated}
          onChange={(event) => field("isolated", event.target.value)}
          style={selectStyle}
        >
          <option value="no">{t("agents.isolation.no")}</option>
          <option value="worktree">{t("agents.isolation.worktree")}</option>
          <option value="copy">{t("agents.isolation.copy")}</option>
        </select>
      </label>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button
          ...
          style={{
            padding: "5px 10px",
            border:
              "1px solid var(--vscode-button-secondaryBorder, var(--border))",
            borderRadius: 2,
...
export function AgentsPanel({ cwd: _cwd }: { cwd: string }): JSX.Element {
  ...
  const reload = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "Unable to load custom agents"
      );
      setAgents([]);
    }
  }, []);
  ...
  const save = async (agent?: AgentDefinitionSummary): Promise<void> => {
    ...
    setError(null);
    try {
      await hostCall("agentSave", {
        name,
        definition: definitionFor(agent?.definition, draft),
      });
      closeEditor();
      await reload();
    } catch (cause) {
    ...
    >
      ...
      >
        <div>
          ...
          >
            Agent YAML definitions available to OMP.
          </p>
        </div>
        <button
          type="button"
          onClick={() => void reload()}
          aria-label={t("agents.action.reload")}
          title={t("agents.action.reload")}
          style={{
            padding: "3px 7px",
            border:
              "1px solid var(--vscode-button-secondaryBorder, var(--border))",
      ...
      {agents?.length === 0 ? (
        ...
        >
          <strong
            style={{ display: "block", color: "var(--text)", fontSize: 13 }}
          >
            {t("agents.empty")}
          </strong>
          <p style={{ margin: "6px 0 12px", fontSize: 12, lineHeight: 1.5 }}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {agents?.map((agent) => {
          const definition = asRecord(agent.definition);
          const model = asRecord(definition.model);
          const expanded = selectedName === agent.name && !newAgent;
          return (
            <article
              key={agent.path}
              style={{
                ...
                border:
                  "1px solid var(--vscode-settings-headerBorder, var(--border))",
                background:
                  "var(--vscode-settings-dropdownBackground, var(--bg-panel))",
                borderRadius: 2,
              }}
            >
              <button
              ...
              >
                ...
                >
                  {t("agents.facts.model")} ={" "}
                  {stringValue(model.default) || "(inherit)"} ·{" "}
                  {t("agents.facts.tools")} ={" "}
                  {toolsValue(definition.tools).length} ·{" "}
                  {t("agents.facts.isolation")} ={" "}
                  {definition.isolated === "worktree" ||
                  definition.isolated === "copy"
                    ? definition.isolated
                    : "no"}
                </p>
              </button>
      ...
      <div style={{ marginTop: 12 }}>
        {newAgent && draft ? (
          ...
          >
            <strong style={{ fontSize: 13 }}>{t("agents.action.new")}</strong>
            <AgentEditor
              draft={draft}
              isNew
              saving={saving}
