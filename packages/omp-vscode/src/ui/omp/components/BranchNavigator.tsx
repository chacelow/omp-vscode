...
function buildActivePath(
  ...
  function search(nodes: SessionTreeNode[], path: string[]): string[] | null {
    for (const node of nodes) {
      ...
      if (
        node.entry.id === target ||
        node.compressedEntryIds?.includes(target)
      ) {
        return next;
      }
...
function compress(node: SessionTreeNode): {
    ...
    <div>
      ...
      <div
        ...
        onContextMenu={(event: MouseEvent<HTMLDivElement>) => {
          ...
          onStartEdit(rep.entry.id, label);
        }}
      >
        {/* Indent guide lines */}
        ...
        {editing ? (
          <input
            ...
            onKeyDown={(event) => {
              if (event.key === "Enter") {
                event.preventDefault();
                onEditLabel(rep.entry.id, draftLabel);
              }
              if (event.key === "Escape") onEditLabel(rep.entry.id, label);
            }}
            style={{
              flex: 1,
              minWidth: 0,
...
export function BranchNavigator({
  ...
  const handleEditLabel = useCallback(
    (entryId: string, label: string) => {
      ...
      void hostCall("sessionRenameEntry", {
        sessionId,
        entryId,
        label: label.trim(),
      });
  ...
  const hasContent =
    !noBranchReason && firstNode && firstNode.children.length > 1;
  const filteredChildren = useMemo(
    () =>
      firstNode?.children.filter((node) => {
        const query = filter.trim().toLowerCase();
        return (
          !query ||
          getLabel(node.entry).toLowerCase().includes(query) ||
  ...
  const onTreeKeyDown = useCallback(
    (event: KeyboardEvent<HTMLDivElement>) => {
          ...
          event.key === "Home"
            ...
            : event.key === "End"
              ...
              : event.key === "ArrowDown" || event.key === "PageDown"
                ? Math.min(
                    ids.length - 1,
                    current + (event.key === "PageDown" ? 5 : 1)
                  )
                : Math.max(0, current - (event.key === "PageUp" ? 5 : 1));
      ...
      if (
        ...
        if (summary?.trim()) {
          void hostCall("sessionAppendSummary", {
            sessionId,
      ...
      if (event.key === "Enter" && activeLeafId) {
        event.preventDefault();
        onLeafChange(activeLeafId);
      }
    },
              ...
              >
                ...
                {filteredChildren.map((child, idx) => (
                  <TreeNodeView
                    ...
                    editingId={editingId}
                    onStartEdit={(entryId) => setEditingId(entryId)}
                  />
                ))}
              </div>
            ) : (
      ...
      {open && (
        <div
          style={{
            ...
            borderBottom: "1px solid var(--border)",
            boxShadow:
              "0 4px 12px var(--vscode-widget-shadow, rgba(0,0,0,0.1))",
            zIndex: 100,
          }}
        >
          {hasContent && firstNode ? (
