...
function toModelOptions(modelList: unknown): ModelOption[] {
  ...
  return Object.entries(modelList as Record<string, unknown>).flatMap(
    ([selector, label]): ModelOption[] => {
      ...
      return [
        {
          ...
          label: typeof label === "string" ? label : selector.slice(slash + 1),
        },
      ];
    }
  );
...
export function ModelRolesPanel({
  ...
  const [assignments, setAssignments] = useState<Record<ModelRole, Assignment>>(
    () =>
      Object.fromEntries(
  ...
  useEffect(() => {
    let mounted = true;
    void Promise.all([
      hostCall("modelsGet", { cwd }),
      hostCall("settingsList" as never, {} as never) as Promise<{
  ...
  return (
    <div
      style={{ color: "var(--text)", background: "var(--bg)", width: "100%" }}
    >
      {MODEL_ROLE_IDS.map((role) => {
        ...
        return (
          <div
            ...
            style={{
              display: "grid",
              gridTemplateColumns:
                "minmax(130px, 1fr) minmax(180px, 2fr) minmax(120px, 1fr)",
              alignItems: "center",
              gap: 12,
              padding: "12px 0",
          ...
          >
            <div>
              ...
              <strong style={{ fontSize: 13 }}>
                {t("modelRole." + role) ?? MODEL_ROLES[role].name}
              </strong>
            </div>
            <select
              aria-label={`${t("modelRole." + role) ?? info.name} ${t("modelRole.modelLabel")}`}
              value={assignment.model}
              onChange={(event) =>
                save(role, { ...assignment, model: event.target.value })
              }
              style={{
                minWidth: 0,
                padding: "6px 8px",
            ...
            <select
              aria-label={`${t("modelRole." + role) ?? info.name} ${t("modelRole.thinkingLabel")}`}
              value={assignment.thinking}
              onChange={(event) =>
                save(role, { ...assignment, thinking: event.target.value })
              }
              style={{
                minWidth: 0,
                padding: "6px 8px",
