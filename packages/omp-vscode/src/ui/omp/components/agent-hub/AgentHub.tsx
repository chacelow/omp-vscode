...
function extensionAgent(value: unknown): AgentDefinitionSummary | null {
  ...
  const definition: AgentDefinition = description
...
export function AgentHub({ cwd, sessionId, onClose }: Props) {
  ...
  const loadAgentDefinitions = useCallback(async () => {
    const [disk, extensions] = await Promise.allSettled([
      hostCall("agentsList", {}),
      ompExtensions(cwd),
    ]);
    const diskAgents = disk.status === "fulfilled" ? disk.value.agents : [];
    const extensionAgents =
      extensions.status === "fulfilled"
        ? extensions.value.extensions
            .map(extensionAgent)
            .filter((agent): agent is AgentDefinitionSummary => agent !== null)
        : [];
    const paths = new Set(diskAgents.map((agent) => agent.path));
    setAgentDefinitions([
      ...diskAgents,
      ...extensionAgents.filter((agent) => !paths.has(agent.path)),
    ]);
  ...
  useEffect(() => {
    let active = true;
    const load = async () => {
      const [disk, native] = await Promise.allSettled([
        hostCall("sessionsList", {}),
        ompSessionsListAll(100),
      ]);
      if (!active) return;
      if (disk.status === "fulfilled") {
        setStored(
          disk.value.sessions.map((entry) => ({
            id: entry.id,
      ...
      if (disk.status === "rejected" && native.status === "rejected")
        setError("Unable to load agent sessions.");
    };
    void load();
  ...
  return (
    ...
    >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {sessions.map((session) => (
            <button
              ...
              className={`block w-full border-b border-[var(--border)] px-3 py-2 text-left text-xs hover:bg-[var(--bg-hover)] ${selected === session.id ? "bg-[var(--bg-hover)]" : ""}`}
            >
              <span className="block truncate font-medium">
                {session.label}
              </span>
              <span className="block truncate text-[10px] text-[var(--text-muted)]">
                {session.origin === "live" ? "LIVE · " : ""}
          ...
          {sessions.length === 0 && (
            <p className="p-3 text-xs text-[var(--text-muted)]">
              No stored or live agent sessions.
            </p>
          )}
        </div>
      </aside>
      ...
      <aside className="flex w-[min(40vw,440px)] min-w-80 flex-col border-l border-[var(--border)]">
        <section
          className="grid grid-cols-2 gap-2 border-b border-[var(--border)] p-3 text-xs"
          aria-label="Agent metrics"
        >
          <span>
            Messages <strong className="block">{metrics.messages}</strong>
          </span>
          <span>
            Input{" "}
          ...
          <span>
            Cost <strong className="block">${metrics.cost.toFixed(4)}</strong>
          </span>
        </section>
        <div className="min-h-0 flex-1">
          <AgentDashboard
            ...
            onDefinitionsChanged={loadAgentDefinitions}
          />
        </div>
      </aside>
    </div>
