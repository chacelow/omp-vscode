...
function ProviderDetail({
  ...
  const handleDiscoverModels = useCallback(async () => {
    ...
    setSelectedModelIds([]);
    try {
      const data = (await hostCall("modelsConfigDiscover", {
        providerName: name,
        provider: { ...provider, models: undefined },
      })) as { models?: DiscoveredModel[]; endpoint?: string; error?: string };
      if (requestId !== discoveryRequestIdRef.current) return;
      if (data.error || !data.models) {
        setDiscoveryState({
          phase: "error",
          message: data.error ?? "Unable to discover models",
        });
        return;
      }
      setDiscoveryState({
...
function ModelDetail({
  ...
  const lookupMetadata = useCallback(async () => {
    ...
    setMetadataMessage("");
    try {
      const data = (await hostCall("modelsConfigMetadata", {
        providerName,
        provider,
        modelId,
      })) as {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      const d = (await hostCall("modelsConfigTest", {
        providerName,
        provider,
        model,
      })) as {
      ...
      if (!d.ok) {
        setTestState({
          phase: "error",
          message: d.error ?? "Unable to test model",
  ...
  const handleCatalogFill = useCallback(async () => {
    ...
    try {
      const data = (await hostCall("modelsConfigCatalog", {
        query,
        provider: providerName,
        ...(provider.baseUrl?.trim()
          ? { baseUrl: provider.baseUrl.trim() }
          : {}),
      })) as { recommendation?: ModelCatalogRecommendation; error?: string };
      if (requestId !== catalogRequestIdRef.current) return;
      if (data.error || !data.recommendation) {
        setCatalogState({
          phase: "error",
          message: data.error ?? "No catalog recommendation found",
        });
        return;
      }
...
function OAuthDetail({
  ...
  const handleLogin = useCallback(() => {
    ...
    setLoginState({
      phase: "error",
      message: "OAuth login is not supported in the VS Code extension",
    });
  }, []);

  const handleLogout = useCallback(async () => {
    await hostCall("authLogout", { provider: provider.id });
    setLoginState({ phase: "idle" });
    onRefresh();
  ...
  const submitCode = useCallback(
    async (token: string, code: string) => {
      ...
      try {
        const d = (await hostCall("authLogin", {
        ...
        if (!d.success || d.error) {
          setLoginState({
            phase: "error",
            message: d.error ?? "Unable to complete login",
          });
          return;
        }
        setInputValue("");
...
function ApiKeyDetail({
  ...
  const handleSave = useCallback(async () => {
    ...
    setSavedOk(false);
    try {
      const d = (await hostCall("authApiKeySet", {
        provider: provider.id,
        apiKey: apiKey.trim(),
      })) as { success?: boolean; error?: string };
      if (!d.success || d.error) {
        setError(d.error ?? "Unable to save API key");
      } else {
        setApiKey("");
        setSavedOk(true);
  ...
  const handleRemove = useCallback(async () => {
    ...
    setError(null);
    try {
      const d = (await hostCall("authApiKeyDelete", {
        provider: provider.id,
      })) as { success?: boolean; error?: string };
      if (!d.success || d.error)
        setError(d.error ?? "Unable to remove API key");
      else onRefresh();
    } catch (e) {
      setError(String(e));
    } finally {
    ...
    >
      <div
        style={{
          ...
          display: "flex",
          flexDirection: "column",
          boxShadow: "0 8px 32px var(--vscode-widget-shadow, rgba(0,0,0,0.22))",
          overflow: "hidden",
        }}
      >
        {/* Search */}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
          {totalCount === 0 ? (
            ...
            >
              {t("No providers match", "未找到匹配的服务商")}
            </div>
          ) : (
            <div
              style={{
              ...
              {showCustom && (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {t("Custom endpoint format", "自定义 Endpoint 格式")}
                    </div>
                  </div>
                  <span
                    style={{
...
export function ModelsConfig({
  ...
  const loadOAuthProviders = useCallback(() => {
    void hostCall("authProvidersList", {})
      .then((d) => setOauthProviders(d.providers as OAuthProvider[]))
      .catch(() => {});
  }, []);

  const loadApiKeyProviders = useCallback(() => {
    void hostCall("authAllProvidersList", {})
      .then((d) => setApiKeyProviders(d.providers as ApiKeyProvider[]))
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    void hostCall("modelsConfigGet", {})
      .then((d) => {
        const configData = d as ModelsJson;
        const normalized = configData.providers
          ? configData
  ...
  const handleSave = useCallback(async () => {
    ...
    setSavedOk(false);
    try {
      const d = (await hostCall("modelsConfigSet", config)) as {
        success?: boolean;
        error?: string;
      };
      if (!d.success || d.error)
        setSaveError(d.error ?? "Unable to save models configuration");
      else {
        setSavedOk(true);
        setTimeout(() => setSavedOk(false), 2000);
      }
          ...
          style={
            embedded
              ...
              : {
                  ...
                  flexDirection: "column",
                  boxShadow:
                    "0 20px 60px rgba(0,0,0,0.32), 0 0 0 1px var(--border)",
                  overflow: "hidden",
                }
          }
        >
          ...
          >
            ...
            <div
              style={{
                ...
                flexShrink: 0,
                background:
                  "color-mix(in srgb, var(--bg-panel) 78%, var(--bg))",
              }}
            >
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                gap: 6,
                transition:
                  "background-color 0.2s ease, color 0.2s ease, transform 0.12s ease",
